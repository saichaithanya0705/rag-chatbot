...
from app.services.providers.nvidia_client import NvidiaClient, NvidiaGenerationResult
...
from app.services.rag.rag_answer_text import (
    ...
    filter_safe_grounding_contexts,
...
from app.services.rag.rag_prompting import (
    build_prompt,
    select_contexts,
)
...
from app.services.providers.web_search_service import WebSearchError, WebSearchOfflineError, WebSearchService
...
class RagService:
    def __init__(
        ...
        *,
        ...
        self._extractive_fallback_service = extractive_fallback_service
        ...
        for attempt_index, include_thinking in enumerate(attempt_order):
            ...
            try:
                ...
                if self._is_interactive_timeout(error) and prepared.contexts:
                    ...
                    return await self.resolve_generation_fallback(
                        prepared.question,
                        prepared.contexts,
                        reason="timeout",
                    )
    ...
    async def prepare_answer(
        ...
        *,
        ...
        if lexical_shortcut is not None:
            ...
            if is_safe_grounding_context(shortcut_context):
                return PreparedAnswer(
                    question=question,
                    prompt="",
                    system_prompt="You answer plainly.",
            ...
            LOGGER.warning("Rejected instruction-shaped lexical shortcut context %s.", shortcut_context.id)

        comparison_contexts = filter_safe_grounding_contexts(
            self._retrieval_engine.comparison_contexts(
                retrieval_query,
                collection_id=collection_id,
                user_id=user_id,
            )
        ...
        pdf_contexts = filter_safe_grounding_contexts(
            [pdf_context_from_chunk(chunk) for chunk in retrieval.chunks]
        )
        tool_call = (
        ...
        if tool_call is not None:
            try:
                ...
                web_contexts = filter_safe_grounding_contexts(
                    self._retrieval_engine.web_contexts_from_results(web_results)
                )
        ...
        contexts = filter_safe_grounding_contexts(
            select_contexts(
                pdf_contexts,
                web_contexts,
                top_k=actual_top_k,
    ...
    def finalize_answer(
        ...
        safe_contexts = filter_safe_grounding_contexts(contexts)
        ...
        citations = extract_citations(normalized_answer, safe_contexts)
        if not citations and safe_contexts and clean_answer:
            citations = derive_citations_from_answer(clean_answer, safe_contexts)
        if contexts and (
            not safe_contexts
            or not citations
            or references_unknown_sources(normalized_answer, safe_contexts)
            or has_unsupported_substantive_claims(normalized_answer, safe_contexts)
        ):
    ...
    def finalize_streamed_answer(
        ...
        *,
        ...
        answer, citations = RagService.finalize_answer(raw_answer, contexts)
        if contexts and not citations and answer == ungrounded_answer_message():
            return RagService.fallback_from_contexts(
                contexts,
    ...
    def fallback_from_contexts(
        ...
        *,
        ...
        safe_contexts = filter_safe_grounding_contexts(contexts)
        fallback = fallback_finalized_answer(
            safe_contexts,
            question=question,
        ...
        if not fallback.citations:
            return FinalizedAnswer(
                answer=ungrounded_answer_message(), citations=[],
                generation_warning="Answer generation failed or could not be grounded, and no relevant fallback evidence was found.",
            )
        answer, citations = RagService.finalize_answer(fallback.answer, safe_contexts)
        return FinalizedAnswer(
            answer=answer, citations=citations,
            generation_warning=fallback.generation_warning,
        )

    async def resolve_generation_fallback(
        self,
        question: str,
        contexts: list[RetrievedContext],
        *,
        ...
        if not safe_contexts:
            return FinalizedAnswer(
                answer=ungrounded_answer_message(),
                citations=[],
                generation_warning="Answer generation failed or only instruction-shaped evidence was available.",
            )
        warning = GENERATION_FALLBACK_WARNINGS[reason]
        try:
            extracted = await self._extractive_fallback_service.answer(question, safe_contexts)
        except Exception:  # noqa: BLE001
            LOGGER.warning(
                "Local extractive fallback failed; using deterministic evidence composition.",
                exc_info=True,
            )
        else:
            if extracted is not None:
                answer, citations = self.finalize_answer(
                    extracted.answer,
                    [extracted.context],
                )
                if citations and answer != ungrounded_answer_message():
                    LOGGER.info(
                        "Local extractive fallback selected cited evidence (score=%.3f).",
                        extracted.score,
                    )
                    return FinalizedAnswer(
                        answer=answer,
                        citations=citations,
                        generation_warning=(
                            f"{warning} A local semantic extraction model selected the cited sentence."
                        ),
                    )
                LOGGER.warning(
                    "Local extractive fallback output did not pass grounding validation; "
                    "using deterministic evidence composition."
                )

        return self.fallback_from_contexts(safe_contexts, reason=reason, question=question)

...
from app.services.rag.rag_types import FinalizedAnswer, RetrievedContext
...
def direct_comparison_shortcut(
    ...
    del question
    contexts = filter_safe_grounding_contexts(list(contexts))
    if len(contexts) < 2:
        return None
...
def direct_context_shortcut(
    ...
    contexts = filter_safe_grounding_contexts(contexts)

...
from app.services.rag.rag_grounding import CONTEXT_FALLBACK_CHAR_LIMIT, clean_context_snippet, normalize_context_text
...
from app.services.rag.rag_types import CandidateChunk, RetrievalResult, RetrievedContext
...
def rank_candidates_by_relevance(
    *,
    ...
    rerank_scores = reranker_service.score_pairs(
        question,
        [candidate.text for candidate in ranked_candidates],
    )
    if len(rerank_scores) != len(ranked_candidates):
        LOGGER.warning("Discarding retrieval candidates: incomplete relevance scores.")
        return []
    for candidate, score in zip(ranked_candidates, rerank_scores, strict=True):
        candidate.rerank_score = score
    ranked_candidates = [
        candidate for candidate in ranked_candidates
        if candidate.rerank_score is not None
        and math.isfinite(candidate.rerank_score)
        and candidate.rerank_score > 0.0
    ]
...
class RagRetrievalEngine:
    ...
    def fallback_chunks(
        ...
        *,
        ...
        reranked = self._rank_candidates(
            question=question, ordered_candidates=all_chunks,
        )[:actual_top_k]
    ...
    def should_use_web_search(self, top_rerank_score: float | None) -> bool:
        if top_rerank_score is None or not math.isfinite(top_rerank_score):
            return True
    ...
    def _rank_candidates(
        ...
        *,
        ...
        return rank_candidates_by_relevance(
            question=question,

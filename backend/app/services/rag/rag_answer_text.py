...
from app.services.rag.rag_citations import citation_from_context
...
UNTRUSTED_EVIDENCE_PATTERN = re.compile(
    r"\b(?:ignore|disregard|override|forget|bypass)\b[^\n.]{0,100}\b(?:previous|prior|above|system|developer)\b[^\n.]{0,80}\b(?:instruction|prompt|rule|policy)s?\b"
    r"|\b(?:system|developer)\s+prompt\b"
    r"|\b(?:reveal|exfiltrate|show|print|disclose)\b[^\n.]{0,100}\b(?:secret|credential|api[ _-]?key|token|prompt)\b",
    re.IGNORECASE,
)
...
CLAIM_CONJUNCTION_PATTERN = re.compile(r"\s+(?:and|but|while|although|however|yet)\s+", re.IGNORECASE)
...
def is_safe_grounding_context(context: RetrievedContext) -> bool:
    """Reject retrieved instructions before they can enter an answering or citation path."""
    return not UNTRUSTED_EVIDENCE_PATTERN.search(context.text)


def filter_safe_grounding_contexts(
    contexts: list[RetrievedContext],
) -> list[RetrievedContext]:
    return [context for context in contexts if is_safe_grounding_context(context)]
...
def substantive_claims(answer: str) -> list[str]:
    claims: list[str] = []
    for segment in substantive_segments(strip_citation_markers(answer)):
        for sentence in re.split(r"(?<=[.!?])\s+", segment):
            candidate = sentence.strip()
            if not TOKEN_PATTERN.search(candidate):
                continue
            clauses = _split_atomic_clauses(candidate)
            claims.extend(clauses or [candidate])
    return claims


def _split_atomic_clauses(sentence: str) -> list[str]:
    clauses = [piece.strip(" ,;:") for piece in CLAIM_CONJUNCTION_PATTERN.split(sentence) if piece.strip()]
    if len(clauses) < 2 or any(len(distinctive_claim_tokens(clause)) < 2 for clause in clauses):
        return [sentence]
    return clauses
...
def context_supports_claim(claim: str, context: RetrievedContext) -> bool:
    claim_tokens = distinctive_claim_tokens(claim)
    if not claim_tokens:
        return False
    context_tokens = distinctive_claim_tokens(context.text)
    return claim_tokens.issubset(context_tokens)


def supporting_contexts_for_segment(
    segment: str,
    contexts: list[RetrievedContext],
) -> list[RetrievedContext]:
    supported: list[RetrievedContext] = []
    for claim in substantive_claims(segment):
        context = best_context_for_segment(claim, contexts)
        if context is None or not context_supports_claim(claim, context):
            return []
...
def has_unsupported_substantive_claims(
    answer: str,
    contexts: list[RetrievedContext],
) -> bool:
    return any(
        not supporting_contexts_for_segment(claim, contexts)
        for claim in substantive_claims(answer)
    )
...
def derive_citations_from_answer(
    ...
    for segment in substantive_segments(answer):
        supporting_contexts = supporting_contexts_for_segment(segment, contexts)

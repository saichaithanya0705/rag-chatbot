...
def _embed_with_local_model(model: Any, texts: list[str]) -> list[list[float]]:
    if hasattr(model, "embed"):
        raw_embeddings = model.embed(texts)
    else:
...
class NvidiaClient:
    def __init__(
        ...
        *,
        ...
        self._default_local_embed_model = None
        self._local_model_lock = threading.RLock()
    ...
    async def embed_texts(
        ...
        *,
        ...
        embeddings = await asyncio.get_running_loop().run_in_executor(
            None,
            lambda: _embed_with_local_model(self._get_local_model(), texts),
        )
        self._validate_embedding_dimensions(embeddings)
        return embeddings

    async def embed_texts_locally(self, texts: list[str]) -> list[list[float]]:
        """Embed text with the default local model without contacting the remote provider."""
        if not texts:
            return []

        embeddings = await asyncio.get_running_loop().run_in_executor(
            None,
            lambda: _embed_with_local_model(self._get_default_local_model(), texts),
        )
        if len(embeddings) != len(texts):
            raise ValueError(
                f"Local fallback model returned {len(embeddings)} embeddings for {len(texts)} texts."
            )
        for index, embedding in enumerate(embeddings):
            if len(embedding) != DEFAULT_LOCAL_EMBEDDING_DIMENSIONS:
                raise ValueError(
                    "Local fallback model returned "
                    f"{len(embedding)} dimensions for item {index}, expected "
                    f"{DEFAULT_LOCAL_EMBEDDING_DIMENSIONS}."
                )
        return embeddings
    ...
    def _get_default_local_model(self) -> Any:
        if (
            not self._embedding_runtime.uses_cloud
            and self._resolved_embed_model == DEFAULT_LOCAL_EMBEDDING_MODEL
        ):
            return self._get_local_model()

        with self._local_model_lock:
            if self._default_local_embed_model is None:
                LOGGER.info(
                    "Lazy loading default local embedding model: %s",
                    DEFAULT_LOCAL_EMBEDDING_MODEL,
                )
                self._default_local_embed_model = _load_local_embedding_model(
                    DEFAULT_LOCAL_EMBEDDING_MODEL,
                    cache_dir=self._local_embedding_cache_dir,
                )
            return self._default_local_embed_model
